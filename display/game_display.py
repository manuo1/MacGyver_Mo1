...
from config.settings import (MAZE_MAP_TEXT_FILE, PATH_IMAGE, MENU_IMAGE,
                             GAME_SCREEN_SIZE, SQUARES_SIZE, END_TEXT_COLOR,
                             END_TEXT)
...
class GameDisplay:
    ...
    def __init__(self, guardian_position, items_to_collect_dict):
        ...
        self.menu_image = pygame.transform.scale(self.menu_image,
                                                 (SQUARES_SIZE*4,
                                                  SQUARES_SIZE*11))

        self.path_image = pygame.image.load(PATH_IMAGE).convert_alpha()
        self.path_image = pygame.transform.scale(self.path_image,
            ...
            for y, line in enumerate(infile):
                ...
                for x, char in enumerate(line):
                    ...
                    if char == 'P'or char == 'S' or char == 'X':
                        ...
                        GameDisplay.maze_blocks_sprites.add(MazeBlockDisplay(x, y, 'PATH'))
                    elif char == 'W':
                        #add to maze_blocks_sprites group as wall
                        GameDisplay.maze_blocks_sprites.add(MazeBlockDisplay(x, y, 'WALL'))
    ...
    def hero_blit(self, hero_position, hero_old_position):
        """apply hero surface on the game screen
           and erase everything on the player's way
        """
        #apply a path image on the game screen at the old hero position
        self.game_screen.blit(self.path_image,
                              [hero_old_position[0]*SQUARES_SIZE,
                               hero_old_position[1]*SQUARES_SIZE])
        #apply hero image at hero position on the game screen
        self.game_screen.blit(self.hero_display.image,
                              [hero_position[0]*SQUARES_SIZE,
                               hero_position[1]*SQUARES_SIZE])

    def unrefreshed_images_blit(self):
        """Apply on the game screen all the sprite groups
           that will not be refreshed.
        """
    ...
    def hero_inventory_blit(self, hero_inventory):
        ...
        #clear menu display
        self.game_screen.blit(self.menu_image,
                              [15*SQUARES_SIZE, 1*SQUARES_SIZE])
        ...
        if hero_inventory:
            ...
            for item in hero_inventory:
                ...
                GameDisplay.hero_inventory_display_sprites.add(ItemToCollectDisplay( 16.5, (hero_inventory.index(item)+2.5), item))
    ...
    def end_game_message(self, hero_win_or_loose):
        ...
        if hero_win_or_loose == 'win' or hero_win_or_loose == 'loose':
            ...
            # draw the end text (anti-aliasing on and colored with orange color) on self.text surface
            self.text = self.font.render(END_TEXT[hero_win_or_loose], 1, END_TEXT_COLOR)
            # resize text surface
            self.text = pygame.transform.scale(self.text, (SQUARES_SIZE*18, SQUARES_SIZE*3))
            # positions the text surface
            self.textpos = self.text.get_rect(centerx=self.game_screen.get_width()//2, centery=((self.game_screen.get_height()//2)-SQUARES_SIZE*2))
